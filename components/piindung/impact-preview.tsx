...
import { ArrowRight, ClipboardCheck, FileText, GitBranch, MonitorCheck, Users, Heart, Shield, BarChart3 } from "lucide-react"
import { LandingCard, LandingReveal } from "@/components/piindung/landing-motion"
import { useImpactPreviewContent, DEFAULT_IMPACT_PREVIEW_CONTENT, type ImpactCardIconKey } from "@/lib/impact-preview-content"

const iconMap: Record<ImpactCardIconKey, React.ElementType> = {
...
export function ImpactPreview() {
  const data = useImpactPreviewContent()
  const d = data ?? DEFAULT_IMPACT_PREVIEW_CONTENT
  const visibleCards = d.cards.filter((c) => c.visible)
      ...
      <div className="mx-auto grid max-w-[1180px] items-start gap-12 px-6 sm:px-10 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16">
        <div className="pt-1 lg:sticky lg:top-28 lg:self-start">
          <LandingReveal delay={0} distance={24} duration={0.7}>
            <p className="text-[10px] font-bold uppercase tracking-[.22em] text-[#07965d]">{d.eyebrow}</p>
          </LandingReveal>
          <LandingReveal delay={0.1} distance={24} duration={0.7}>
            <h2 id="impact-heading" className="mt-4 text-[clamp(2.4rem,4vw,3.65rem)] font-bold leading-[1.03] tracking-[-.06em] text-[#08213b] dark:text-white">
              {d.headingLine1}{d.headingLine2 ? <><br />{d.headingLine2}</> : null}{" "}
              <span className="text-[#07965d]">{d.headingAccent}</span>
            </h2>
          </LandingReveal>
          <LandingReveal delay={0.2} distance={24} duration={0.7}>
            <p className="mt-6 max-w-[460px] text-[15px] leading-8 text-[#6c7a89] dark:text-slate-300">{d.description}</p>
          </LandingReveal>
          {d.quote && (
            <LandingReveal delay={0.3} distance={24} duration={0.7}>
              <div className="mt-7 flex max-w-[460px] gap-3 rounded-[20px] border border-[#dce8e2] bg-white p-5 shadow-[0_10px_28px_rgba(9,43,32,.055)] dark:border-white/10 dark:bg-[#0d1e2d]">
                <p className="text-sm leading-7 text-[#08213b] dark:text-slate-200">{d.quote}</p>
              </div>
            </LandingReveal>
          )}
          ...
          {visibleCards.map((card, index) => {
            ...
            return (
              <LandingCard key={card.id} as="article" revealIndex={index} revealColumns={2} className="group flex min-h-[250px] flex-col rounded-[24px] border border-[#dce8e2] bg-white p-7 shadow-[0_12px_32px_rgba(9,43,32,.065)] transition-colors duration-300 hover:border-[#07965d]/45 hover:shadow-[0_20px_46px_rgba(9,43,32,.12)] dark:border-white/10 dark:bg-[#0d1e2d]">
                ...
                <h3 className="mt-8 min-h-[56px] text-xl font-bold leading-7 tracking-[-.035em] text-[#08213b] dark:text-white">{card.title}</h3>
                <p className="mt-auto pt-3 text-sm leading-7 text-[#6c7a89] dark:text-slate-300">{card.description}</p>
              </LandingCard>
            )
          })}
        </div>
