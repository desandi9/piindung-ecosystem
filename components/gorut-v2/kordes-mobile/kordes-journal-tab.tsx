...
export function KordesJournalTab({ batches, village, kecamatan, onOpenRecap }: { batches: CollectionBatch[]; village: string; kecamatan: string; onOpenRecap: (period: string) => void }) {
  ...
  return <><MobilePageHeader title="Jurnal Kordes" subtitle={`Rekap Desa ${village}`} /><div className="plpk-scroll">
    ...
    <div className="plpk-journal-list kordes-journal-list">{visible.map((item) => {
      const financialReady = item.batches.every((batch) => !isServerCollectionBatch(batch) || batch.canonical.financial.status === 'READY');
      return <article key={item.period} className="plpk-journal-card">
      ...
      <div className="plpk-journal-amount"><span><MobileServiceIcon icon={Coins01Icon} label="Jumlah bersih" size={19} /></span><div><small>Jumlah bersih ranting</small><strong>{financialReady ? formatRupiah(item.netAmount) : 'Belum siap'}</strong></div></div>
      ...
      {expanded === item.period ? <dl className="plpk-journal-details"><div><dt>Total Munfiq</dt><dd>{formatNumber(item.munfiqCount)}</dd></div><div><dt>Jumlah kotor</dt><dd>{financialReady ? formatRupiah(item.grossAmount) : 'Belum siap'}</dd></div><div><dt>Total bisyaroh</dt><dd>{financialReady ? formatRupiah(item.totalPlpkFee) : 'Belum siap'}</dd></div><div className="is-total"><dt>Jumlah bersih</dt><dd>{financialReady ? formatRupiah(item.netAmount) : 'Belum siap'}</dd></div><div><dt>Status verifikasi</dt><dd>{statusLabels[item.recapStatus]}</dd></div><div><dt>Status F.010</dt><dd>{item.f010Status}</dd></div><div><dt>Status F.015</dt><dd>{item.f015Ready ? 'Siap' : 'Belum Siap'}</dd></div></dl> : null}
