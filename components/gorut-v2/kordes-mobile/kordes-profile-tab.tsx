...
import { formatPhoneNumber } from '@/features/gorut-v2/formatters';
...
export function KordesProfileTab({ profile, plpkCount, onNotice, onLogout, logoutPending = false }: { profile: KordesHomeProfile; plpkCount: number; onNotice: (message: string) => void; onLogout: () => void; logoutPending?: boolean }) {
  ...
  return <><MobilePageHeader title="Profil" subtitle="Akun dan pengaturan Kordes" /><div className="plpk-scroll"><section className="plpk-profile-hero"><div className="plpk-profile-avatar" aria-hidden="true">{profile.identity.initials}</div><div><h1>{profile.name}</h1><span>{profile.rantingCode}</span><small>{profile.identity.assignmentLabel}</small></div><span className="plpk-account-badge">Akun Aktif</span></section>
    <MobileSectionHeader title="Informasi Kordes" /><section className="plpk-card"><dl className="plpk-profile-rows"><div><dt>Kode Ranting</dt><dd>{profile.rantingCode}</dd></div><div><dt>Ranting/desa</dt><dd>{profile.village}</dd></div><div><dt>UPZIS/kecamatan</dt><dd>{profile.upzis}, {profile.kecamatan}</dd></div><div><dt>Nomor HP</dt><dd>{formatPhoneNumber(profile.phone)}</dd></div><div><dt>PLPK binaan</dt><dd>{plpkCount} petugas</dd></div><div><dt>Status akun</dt><dd><span className="plpk-account-inline">Aktif</span></dd></div></dl></section>
    ...
    {copy ? <section className="plpk-profile-info"><MobileServiceIcon icon={UserIcon} label={copy[0]} size={20} /><div><strong>{copy[0]}</strong><p>{copy[1]}</p></div></section> : null}<div className="plpk-app-version"><span>Versi aplikasi</span><strong>2.0.0-prototype</strong></div><button type="button" className="plpk-logout-button" onClick={onLogout} disabled={logoutPending}><MobileServiceIcon icon={Logout03Icon} label="Keluar" size={19} />{logoutPending ? 'Sedang keluar…' : 'Keluar'}</button>
  </div></>;
}
