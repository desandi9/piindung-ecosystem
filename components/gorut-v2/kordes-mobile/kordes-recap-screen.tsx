...
export function KordesRecapScreen({ batches, period, village, kecamatan, onBack }: { batches: CollectionBatch[]; period: string; village: string; kecamatan: string; onBack: () => void }) {
  ...
  const financialReady = rows.every((batch) => !isServerCollectionBatch(batch) || batch.canonical.financial.status === 'READY');
  return <><MobilePageHeader title="Rekap Ranting" subtitle={formatPeriodLabel(period)} onBack={onBack} /><div className="plpk-scroll">
    <section className="kordes-recap-totals"><div><small>Jumlah PLPK</small><strong>{formatNumber(rows.length)}</strong></div><div><small>Munfiq aktif</small><strong>{formatNumber(totals.active)}</strong></div><div><small>Terjemput</small><strong>{formatNumber(totals.collected)}</strong></div><div><small>Tidak terjemput</small><strong>{formatNumber(totals.uncollected)}</strong></div><div className="is-wide"><small>Total jumlah kotor</small><strong>{financialReady ? formatRupiah(totals.gross) : 'Belum siap'}</strong></div><div><small>Total bisyaroh</small><strong>{financialReady ? formatRupiah(totals.fee) : 'Belum siap'}</strong></div><div><small>Total bersih</small><strong>{financialReady ? formatRupiah(totals.net) : 'Belum siap'}</strong></div></section>
    {rows.length ? <div className="kordes-recap-list">{rows.map((batch) => <article key={batch.id}><div><strong>{batch.plpkName}</strong><span>{batch.plpkId}</span></div><MobileStatusBadge status={batch.status} /><dl><div><dt>Munfiq aktif</dt><dd>{batch.activeCanCount}</dd></div><div><dt>Terjemput</dt><dd>{batch.collectedCanCount}</dd></div><div><dt>Tidak terjemput</dt><dd>{batch.uncollectedCanCount}</dd></div><div><dt>Jumlah kotor</dt><dd>{collectionMoneyLabel(batch, 'grossAmount')}</dd></div><div><dt>Bisyaroh</dt><dd>{collectionMoneyLabel(batch, 'totalPlpkFee')}</dd></div><div className="is-total"><dt>Jumlah bersih</dt><dd>{collectionMoneyLabel(batch, 'netAmount')}</dd></div></dl></article>)}</div> : <MobileEmptyState icon={Analytics02Icon} title="Rekap belum tersedia" description="Belum ada data PLPK pada periode ini." />}
