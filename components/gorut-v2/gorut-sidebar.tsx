...
export function GorutSidebar({ target }: GorutSidebarProps) {
  ...
  return <aside className={`gorut-sidebar gorut-entrance-sidebar ${isCollapsed ? 'is-collapsed' : ''}`} data-collapse-ready={collapsed !== null}>
    ...
    <button type="button" className="gorut-account-switcher" title={isCollapsed ? 'Admin PC · PC LAZISNU Garut' : undefined}><span className="gorut-avatar">AP</span><span className="gorut-account-copy"><strong>Admin PC</strong><small>PC LAZISNU Garut</small></span><ChevronDown className="gorut-account-caret" size={14} /></button>
