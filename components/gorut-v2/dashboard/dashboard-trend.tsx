  ...
  return (
    <section className="gorut-command-trend" aria-labelledby="gorut-trend-title">
      <header className="gorut-command-section-head">
        <div><h2 id="gorut-trend-title">Tren jumlah bersih</h2><p>Perbandingan enam periode terakhir dari data batch.</p></div>
      ...
      <div className="gorut-command-trend-summary">
        <span>Total enam periode</span><strong>{formatRupiahCompact(totalNet)}</strong>
        ...
        <div className={`gorut-command-chart is-${mode}`}>
          {mode === 'line' ? (
            ...
            <div className="gorut-command-bars-stage">
              {points.map((point) => (
                <button key={point.item.period} type="button" aria-label={`${formatPeriodLabel(point.item.period)}, jumlah bersih ${formatRupiah(point.item.netAmount)}`}>
                  <span className="gorut-command-chart-tooltip" role="tooltip">{formatRupiah(point.item.netAmount)}</span>
                  <i style={{ '--gorut-bar-height': `${(point.item.netAmount / point.max) * 100}%` } as CSSProperties} />
