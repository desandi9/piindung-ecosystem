...
export default function ArticleMigrationPreviewPage() {
  ...
  useEffect(() => {
    if (!isLoading && !user) router.replace("/login?next=/dashboard/landing-page/artikel/migrasi")
...
const summaryItems: Array<[string, LucideIcon, string]> = [
  ...
  return <MemberLayout title="Migrasi Artikel" breadcrumb="Landing Page / Artikel / Migrasi">
    <div className="space-y-7 overflow-x-hidden">
      <motion.section variants={reveal} initial="hidden" animate="visible" className="rounded-[28px] border border-border bg-card p-6 shadow-sm sm:p-8">
        <button onClick={() => router.push("/dashboard/landing-page/artikel")} className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Kembali</button>
