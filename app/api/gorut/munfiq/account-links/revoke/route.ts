...
import { GorutMunfiqIdentityError, revokeUserGorutMunfiqLink } from "@/lib/gorut-munfiq-identity-server"
...
export async function POST(request: Request) {
  ...
  if (!input) return json({ error: "Payload revoke identitas Munfiq tidak valid." }, 400)
  try {
    return json({ link: await revokeUserGorutMunfiqLink(getPrismaClient(), required.access.user.id, input) })
  } catch (error) {
    if (!(error instanceof GorutMunfiqIdentityError)) return json({ error: "Revoke link identitas Munfiq tidak dapat diproses." }, 500)
