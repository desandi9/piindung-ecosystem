...
import { Eye, MapPin, Plus, RotateCcw, Search, X, Pencil } from "lucide-react"
...
import { formatUserOperationalScope, getUserOperationalScopeStatus, gorutScopedOperationalRoles, operationalScopeStatusClass, refreshUserOperationalScopes, upsertUserOperationalScope, useUserOperationalScopes } from "@/lib/user-operational-scope"
...
type ScopeForm = { gorutKecamatan: string; gorutWilayahLabel: string }
...
export default function MemberUsersPage() {
  ...
  const [scopeForm, setScopeForm] = useState<ScopeForm>({ gorutKecamatan: "", gorutWilayahLabel: "" })
  const [scopeFormError, setScopeFormError] = useState("")
  const [isSavingScope, setIsSavingScope] = useState(false)
  const [confirmStaleScope, setConfirmStaleScope] = useState(false)
  ...
  const handleMutationConfirm = async () => {
    ...
    setMutationError("")
    try {
      await updateManagedUser(mutationTarget.id, mutationKind === "role" ? { role: mutationValue as UserRole } : { status: mutationValue as "Aktif" | "Nonaktif" })
  ...
  const openScopeDialog = (targetUser: ManagedUser) => {
    const scope = scopeByUser.get(targetUser.id)
    setScopeTarget(targetUser)
    setScopeForm({
      gorutKecamatan: scope?.gorutKecamatan ?? "",
      gorutWilayahLabel: scope?.gorutWilayahLabel ?? "",
    })
    setScopeFormError("")
    setConfirmStaleScope(false)
    setSaveMessage("")
  }

  const handleScopeSubmit = async (event: FormEvent) => {
    event.preventDefault()
    if (!scopeTarget) return

    const gorutKecamatan = scopeForm.gorutKecamatan.trim()
    const gorutWilayahLabel = scopeForm.gorutWilayahLabel.trim()
    const hasLocalScope = Boolean(gorutKecamatan || gorutWilayahLabel)

    setScopeFormError("")
    if ((scopeTarget.role === "super_admin_pc" || scopeTarget.role === "admin_pc") && hasLocalScope) {
      setScopeFormError("Role global tidak dapat menerima scope lokal baru.")
      return
    }
    if (gorutScopedOperationalRoles.includes(scopeTarget.role) && !gorutKecamatan) {
      setScopeFormError("Kecamatan wajib dipilih dari daftar resmi.")
      return
    }
    if (gorutKecamatan && !gorutKecamatanOptions.includes(gorutKecamatan as (typeof gorutKecamatanOptions)[number])) {
      setScopeFormError("Kecamatan tidak valid. Pilih dari daftar resmi.")
      return
    }
    if (scopeTarget.role === "admin_kordes" && !gorutWilayahLabel) {
      setScopeFormError("Desa/Kelurahan atau Unit wajib diisi untuk Admin Kordes / Ranting.")
      return
    }

    setIsSavingScope(true)
    try {
      const hasStaleGlobalScope = (scopeTarget.role === "super_admin_pc" || scopeTarget.role === "admin_pc") && Boolean(getScope(scopeTarget)?.gorutKecamatan?.trim() || getScope(scopeTarget)?.gorutWilayahLabel?.trim())
      if (hasStaleGlobalScope && !confirmStaleScope) {
        setScopeFormError("Konfirmasi penghapusan scope lokal lama sebelum melanjutkan.")
        return
      }
      await upsertUserOperationalScope({
        userId: scopeTarget.id,
        role: scopeTarget.role,
        gorutKecamatan: gorutScopedOperationalRoles.includes(scopeTarget.role) ? gorutKecamatan : undefined,
        gorutWilayahLabel: gorutScopedOperationalRoles.includes(scopeTarget.role) && gorutWilayahLabel ? gorutWilayahLabel : undefined,
      }, { clearStaleLocalScope: hasStaleGlobalScope, confirmStaleScope: confirmStaleScope })
      await refreshUserOperationalScopes()
      setSaveMessage("Penempatan operasional berhasil diperbarui.")
      setScopeTarget(null)
    } catch (error) {
      setScopeFormError(error instanceof Error ? error.message : "Gagal menyimpan penempatan operasional.")
    } finally {
  ...
  const getScope = (targetUser: ManagedUser) => scopeByUser.get(targetUser.id)
  const organizationLabel = (targetUser: ManagedUser) => formatUserOperationalScope(targetUser.role, getScope(targetUser))
  const scopeStatus = (targetUser: ManagedUser) => getUserOperationalScopeStatus(targetUser.role, getScope(targetUser))
  const scopeQualityIssues = useMemo(() => users.flatMap((targetUser) => {
    const scope = getScope(targetUser)
    const issues: string[] = []
    if (targetUser.role === "admin_upzis" || targetUser.role === "admin_kordes") {
      if (!scope?.gorutKecamatan?.trim()) issues.push(`${targetUser.name}: kecamatan belum ditentukan`)
      if (!scope?.gorutWilayahLabel?.trim()) issues.push(`${targetUser.name}: label wilayah/unit belum tersedia`)
      if (scope?.gorutKecamatan && !gorutKecamatanOptions.includes(scope.gorutKecamatan as (typeof gorutKecamatanOptions)[number])) issues.push(`${targetUser.name}: kecamatan tidak dikenal`)
    }
    if (scope && scope.role !== targetUser.role) issues.push(`${targetUser.name}: role scope berbeda dari role pengguna`)
    return issues
  }), [scopes, users])
    ...
    <MemberLayout title="Pengguna & Akses" breadcrumb="Member Area / Pengguna & Akses">
      <div className="space-y-7 overflow-x-hidden">
        ...
        <motion.section variants={reduced ? { hidden: {}, visible: {} } : staggerContainer} initial="hidden" animate="visible" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
         ...
         {scopeQualityIssues.length > 0 && <section className="rounded-[22px] border border-amber-200 bg-amber-50/70 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10" aria-label="Audit kualitas scope"><p className="font-semibold text-amber-800 dark:text-amber-200">Audit scope operasional</p><p className="mt-1 text-amber-700 dark:text-amber-300">{scopeQualityIssues.length} temuan terdeteksi. Data tidak diubah otomatis.</p><ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-amber-700 dark:text-amber-300">{scopeQualityIssues.slice(0, 5).map((issue) => <li key={issue}>{issue}</li>)}</ul>{scopeQualityIssues.length > 5 && <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">+{scopeQualityIssues.length - 5} temuan lainnya.</p>}</section>}
        ...
        <motion.section variants={reduced ? { hidden: {}, visible: {} } : staggerContainer} initial="hidden" animate="visible" className="grid gap-4 md:hidden" aria-label="Daftar pengguna mobile">
          {filteredUsers.map((user) => <motion.article key={user.id} variants={itemReveal} className="rounded-[20px] border border-border bg-card p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><UserIdentity user={user} /><span className={cn("shrink-0 rounded-full px-3 py-1 text-xs font-semibold", statusClass(user.status))}>{user.status}</span></div><dl className="mt-5 grid grid-cols-1 gap-4 text-sm sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">Role</dt><dd className="mt-1 font-medium text-foreground">{roleDisplayNames[user.role] || user.role}</dd></div><div><dt className="text-xs text-muted-foreground">Organisasi/Wilayah</dt><dd className="mt-1 space-y-1 font-medium text-foreground"><p className="break-words">{organizationLabel(user)}</p><span className={cn("inline-flex rounded-full px-2 py-0.5 text-xs font-semibold", operationalScopeStatusClass(scopeStatus(user)))}>{scopeStatus(user)}</span></dd></div></dl>{user.id !== currentUserId && <div className="mt-4 grid grid-cols-2 gap-2"><select value={user.role} onChange={(event) => openRoleMutation(user, event.target.value as UserRole)} className="h-11 min-w-0 rounded-xl border border-border bg-background px-2 text-xs font-medium text-foreground" aria-label={`Ubah role ${user.name}`}>{roleOptions.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select><select value={user.status} onChange={(event) => openStatusMutation(user, event.target.value as "Aktif" | "Nonaktif")} className="h-11 min-w-0 rounded-xl border border-border bg-background px-2 text-xs font-medium text-foreground" aria-label={`Ubah status ${user.name}`}>{user.status === "Menunggu" && <option value="Menunggu" disabled>Menunggu</option>}<option value="Aktif">Aktif</option><option value="Nonaktif">Nonaktif</option></select></div>}<div className="mt-5 grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => setSelectedUser(user)} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#e6f7ee] text-sm font-semibold text-[#15945b] dark:bg-emerald-500/10 dark:text-emerald-400"><Eye className="h-4 w-4" /> Lihat Detail</button><button type="button" onClick={() => openScopeDialog(user)} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-border text-sm font-semibold text-muted-foreground"><MapPin className="h-4 w-4" /> Atur Penempatan</button><button type="button" onClick={() => handleOpenEdit(user)} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-border text-sm font-semibold text-muted-foreground"><Pencil className="h-4 w-4" /> Edit</button></div></motion.article>)}
        </motion.section>
        ...
        {selectedUser && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedUser(null) }}><motion.aside initial={reduced ? { opacity: 1 } : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-h-[90vh] w-full overflow-y-auto rounded-t-[24px] border border-border bg-card p-6 shadow-2xl sm:max-w-lg sm:rounded-[24px]" role="dialog" aria-modal="true" aria-labelledby="user-detail-title"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#15945b]">DETAIL PENGGUNA</p><h2 id="user-detail-title" className="mt-2 text-2xl font-bold text-foreground">{selectedUser.name}</h2></div><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => openScopeDialog(selectedUser)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-border px-3 text-sm font-semibold text-muted-foreground hover:bg-accent"><MapPin className="h-4 w-4" /> Atur Penempatan</button><button type="button" onClick={() => handleOpenEdit(selectedUser)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-border px-3 text-sm font-semibold text-muted-foreground hover:bg-accent"><Pencil className="h-4 w-4" /> Edit</button><button type="button" onClick={() => setSelectedUser(null)} className="inline-flex h-10 w-10 items-center justify-center rounded-xl text-muted-foreground hover:bg-accent" aria-label="Tutup detail"><X className="h-5 w-5" /></button></div></div><div className="mt-6 flex items-center gap-4"><UserIdentity user={selectedUser} /></div><dl className="mt-6 space-y-4 text-sm"><div className="flex justify-between gap-4 border-b border-border pb-3"><dt className="text-muted-foreground">Role</dt><dd className="font-semibold text-foreground">{roleDisplayNames[selectedUser.role] || selectedUser.role}</dd></div><div className="flex justify-between gap-4 border-b border-border pb-3"><dt className="text-muted-foreground">Status</dt><dd><span className={cn("rounded-full px-3 py-1 text-xs font-semibold", statusClass(selectedUser.status))}>{selectedUser.status}</span></dd></div><div className="flex flex-col gap-2 border-b border-border pb-3 sm:flex-row sm:justify-between sm:gap-4"><dt className="text-muted-foreground">Organisasi/Wilayah</dt><dd className="break-words font-semibold text-foreground sm:text-right">{organizationLabel(selectedUser)}</dd></div></dl><section className="mt-6 rounded-xl border border-border bg-muted/30 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold text-foreground">Penempatan Operasional</h3><span className={cn("rounded-full px-2 py-0.5 text-xs font-semibold", operationalScopeStatusClass(scopeStatus(selectedUser)))}>{scopeStatus(selectedUser)}</span></div><dl className="mt-4 space-y-3 text-sm"><div className="grid gap-1 sm:grid-cols-[140px_minmax(0,1fr)] sm:gap-4"><dt className="text-muted-foreground">Cakupan akses</dt><dd className="break-words font-medium text-foreground">{organizationLabel(selectedUser)}</dd></div>{getScope(selectedUser)?.gorutKecamatan?.trim() && <div className="grid gap-1 sm:grid-cols-[140px_minmax(0,1fr)] sm:gap-4"><dt className="text-muted-foreground">Kecamatan</dt><dd className="break-words font-medium text-foreground">{getScope(selectedUser)?.gorutKecamatan}</dd></div>}{getScope(selectedUser)?.gorutWilayahLabel?.trim() && <div className="grid gap-1 sm:grid-cols-[140px_minmax(0,1fr)] sm:gap-4"><dt className="text-muted-foreground">Wilayah/unit</dt><dd className="break-words font-medium text-foreground">{getScope(selectedUser)?.gorutWilayahLabel}</dd></div>}<div className="grid gap-1 sm:grid-cols-[140px_minmax(0,1fr)] sm:gap-4"><dt className="text-muted-foreground">Relasi role</dt><dd className="font-medium text-foreground">{roleDisplayNames[getScope(selectedUser)?.role || selectedUser.role] || getScope(selectedUser)?.role || selectedUser.role}</dd></div></dl></section><dl className="mt-4 space-y-4 text-sm">{selectedUser.email && <div className="flex justify-between gap-4 border-b border-border pb-3"><dt className="text-muted-foreground">Email</dt><dd className="max-w-[60%] break-all text-right font-semibold text-foreground">{selectedUser.email}</dd></div>}<div className="flex justify-between gap-4"><dt className="text-muted-foreground">Terakhir Aktif</dt><dd className="text-right font-semibold text-foreground">{selectedUser.lastLogin || "—"}</dd></div></dl></motion.aside></div>}

        {scopeTarget && <div className="fixed inset-0 z-[55] flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setScopeTarget(null) }}><motion.form initial={reduced ? { opacity: 1 } : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleScopeSubmit} className="max-h-[90vh] w-full overflow-y-auto rounded-t-[24px] border border-border bg-card p-6 shadow-2xl sm:max-w-lg sm:rounded-[24px]"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#15945b]">ATUR PENEMPATAN</p><h2 className="mt-2 text-2xl font-bold text-foreground">{scopeTarget.name}</h2></div><button type="button" onClick={() => setScopeTarget(null)} className="inline-flex h-10 w-10 items-center justify-center rounded-xl text-muted-foreground hover:bg-accent" aria-label="Tutup penempatan"><X className="h-5 w-5" /></button></div>{scopeFormError && <div className="mt-5 rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-sm text-destructive">{scopeFormError}</div>}<div className="mt-6 space-y-4"><div className="rounded-xl border border-border bg-muted/30 p-4 text-sm"><div className="grid gap-3"><div><p className="text-xs text-muted-foreground">Nama pengguna</p><p className="mt-1 font-semibold text-foreground">{scopeTarget.name}</p></div><div><p className="text-xs text-muted-foreground">Role</p><p className="mt-1 font-semibold text-foreground">{roleDisplayNames[scopeTarget.role] || scopeTarget.role}</p></div><div><p className="text-xs text-muted-foreground">Penempatan saat ini</p><p className="mt-1 break-words font-semibold text-foreground">{organizationLabel(scopeTarget)}</p></div></div></div>{(scopeTarget.role === "super_admin_pc" || scopeTarget.role === "admin_pc") ? <div className="space-y-3"><div className="rounded-xl border border-amber-200 bg-amber-50/70 p-4 text-sm text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200"><p className="font-semibold">Role ini menggunakan cakupan global.</p>{getScope(scopeTarget)?.gorutKecamatan || getScope(scopeTarget)?.gorutWilayahLabel ? <><p className="mt-1">Terdapat scope lokal lama pada data. Scope tersebut tidak dihapus otomatis.</p><label className="mt-3 flex items-start gap-3"><input type="checkbox" checked={confirmStaleScope} onChange={(event) => setConfirmStaleScope(event.target.checked)} className="mt-1 h-4 w-4 rounded border-border text-[#15945b]" /><span>Saya mengonfirmasi penghapusan scope lokal lama.</span></label></> : <p className="mt-1">Tidak memerlukan penempatan lokal tambahan.</p>}</div></div> : <><label className="space-y-2"><span className="text-sm font-medium text-foreground">Kecamatan</span><select value={scopeForm.gorutKecamatan} onChange={(event) => setScopeForm((current) => ({ ...current, gorutKecamatan: event.target.value }))} className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-[#15945b]" required><option value="">Pilih kecamatan</option>{gorutKecamatanOptions.map((kecamatan) => <option key={kecamatan} value={kecamatan}>{kecamatan}</option>)}</select></label>{scopeTarget.role === "admin_kordes" && <label className="space-y-2"><span className="text-sm font-medium text-foreground">Desa/Kelurahan atau Unit</span><input value={scopeForm.gorutWilayahLabel} onChange={(event) => setScopeForm((current) => ({ ...current, gorutWilayahLabel: event.target.value }))} className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none focus:border-[#15945b]" required /></label>}{scopeTarget.role === "admin_upzis" && <label className="space-y-2"><span className="text-sm font-medium text-foreground">Wilayah/Unit</span><input value={scopeForm.gorutWilayahLabel} onChange={(event) => setScopeForm((current) => ({ ...current, gorutWilayahLabel: event.target.value }))} className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none focus:border-[#15945b]" placeholder="Opsional bila didukung data" /></label>}</>}</div><div className="mt-6 flex flex-col gap-3"><button type="button" onClick={() => setScopeTarget(null)} disabled={isSavingScope} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-border px-4 text-sm font-semibold text-muted-foreground">Batal</button>{gorutScopedOperationalRoles.includes(scopeTarget.role) && <button type="submit" disabled={isSavingScope} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-[#15945b] px-5 text-sm font-semibold text-white disabled:opacity-60">{isSavingScope ? "Menyimpan..." : "Simpan Penempatan"}</button>}</div></motion.form></div>}
