...
function ProgressList({ title, items }: { title: string; items: MonitoringProgressItem[] }) {
  return (
    <Card className="border-border/60 bg-card shadow-sm">
      <CardHeader className="pb-3"><CardTitle className="text-base">{title}</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        {items.length === 0 ? <EmptyState inline icon={Activity} title="Belum ada data progress" description="Data akan tampil setelah transaksi GORUT masuk database normalized." /> : items.slice(0, 5).map((item) => (
          <div key={item.id} className="rounded-2xl border border-border/60 bg-background/40 p-4 transition-colors hover:bg-muted/30">
            <div className="mb-3 flex items-start justify-between gap-3">
              ...
              <Badge variant="outline" className="min-h-7 shrink-0 text-xs">{item.progress}%</Badge>
            </div>
            <MetricBar label={`${item.finalApprovedCount} selesai • ${item.pendingCount} pending`} value={formatRupiah(item.finalApprovedAmount)} width={item.progress} tone={item.pendingCount > 0 ? 'amber' : 'emerald'} />
          </div>
        ))}
      </CardContent>
    </Card>
...
export default function SystemMonitoringPage() {
  const snapshot = useGorutMonitoringSnapshot()
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="border-border/60 bg-card shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <MetricBar label="Bulan Ini" value={formatRupiah(snapshot.finalApprovedTotals.currentMonthAmount)} width={snapshot.finalApprovedTotals.totalAmount ? (snapshot.finalApprovedTotals.currentMonthAmount / snapshot.finalApprovedTotals.totalAmount) * 100 : 0} tone="blue" />
