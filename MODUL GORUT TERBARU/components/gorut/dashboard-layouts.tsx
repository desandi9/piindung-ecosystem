...
export function DashboardPageHeader({
  ...
  return (
    <div className={cn('flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between', className)}>
      ...
      {action && <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end [&_button]:min-h-[44px]">{action}</div>}
    </div>
  )
}
    ...
    <div className={cn('space-y-4', className)}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-1">
          ...
          {description && <p className="text-sm leading-6 text-muted-foreground sm:text-xs">{description}</p>}
        </div>
        {headerAction && <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end [&_button]:min-h-[44px]">{headerAction}</div>}
      </div>
      {children}
    </div>
  )
...
export function ContentCard({
  ...
  return (
    <Card className={cn('border-0 bg-card shadow-sm', className)}>
      {(title || description) && (
        <CardHeader className="flex flex-col gap-3 border-b border-border/50 pb-4 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          <div className="min-w-0 space-y-1">
            {title && <CardTitle className="text-base sm:text-lg">{title}</CardTitle>}
            {description && <CardDescription className="text-sm leading-6 sm:text-sm">{description}</CardDescription>}
          </div>
          {headerAction && <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end [&_button]:min-h-[44px]">{headerAction}</div>}
        </CardHeader>
      )}
...
export function ContentGrid({
  ...
  const gridClass = {
    auto: 'grid gap-4 sm:gap-6 lg:grid-cols-3',
    '2': 'grid gap-4 sm:gap-6 lg:grid-cols-2',
    '3': 'grid gap-4 sm:gap-6 lg:grid-cols-3',
  }
